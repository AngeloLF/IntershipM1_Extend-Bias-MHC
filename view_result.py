...
def view(folder, case, langage):
	...
	folder_result = f"{folder}_RESULT"
	...
	df = pd.read_csv(f"{folder}/{case}/{langage}/predictions.csv")
	...
	INDEX = ['0_Numbers', '1_ACC', '1_AUC', '1_MCC', '2_H-F1', '2_NH-F1', '2_MacF1', '2_F1-AUC']
	...
	INDEX += [f"MCC_{ident}" for ident in idents]
	...
	# Precision & Recall
	ppv, ppv_nh = tp / (tp + fp), tp_nh / (tp_nh + fp_nh)
	tpr, tpr_nh = tp / (tp + fn), tp_nh / (tp_nh + fn_nh)

	# F1 score, accuracy, MCC
	f1, f1_nh = 2 * (ppv * tpr) / (ppv + tpr), 2 * (ppv_nh * tpr_nh) / (ppv_nh + tpr_nh)
	acc = (tp + tn) / (p + n)
	...
	data_result[INDEX.index(f"1_MCC")]     = np.round(mcc*100, 2)
	...
	for i, ident in enumerate(idents):
		...
		ppv_nh = tp_nh / (tp_nh + fp_nh)
		tpr_nh = tp_nh / (tp_nh + fn_nh)

		id_acc = (tp + tn) / n_tot
		id_mcc = (tp*tn - fp*fn) / (np.sqrt(tp+fp) * np.sqrt(tp+fn) * np.sqrt(tn+fp) * np.sqrt(tn+fn))
		id_mm = 1 - fn / p
		id_op = 1 - fp / n

		id_f1 = 2 * (ppv * tpr) / (ppv + tpr)
		id_f1_nh = 2 * (ppv_nh * tpr_nh) / (ppv_nh + tpr_nh)
		...
		data_result[INDEX.index(f"MCC_{ident}")] = np.round(id_mcc*100, 2) # MCC
...
def GMB(scor, p=-5):
	...
	return (1/len(scor) * np.sum(scor**p)) ** (1/p)
...
def make_graph(folder, case, cmap=['RdYlGn', 'PRGn', 'RdYlGn'], fmt='.1f', linewidths=1.0):
	...
	df = pd.read_csv(f"{folder}_RESULT/{case}_RESULT/{case}_RESULT.csv", index_col=0)
	...
	mcc = [indi for indi in ind if 'MCC_' in indi]
	...
	for pGMBi in pGMB:
		...
		if f"GMB p={pGMBi}" not in os.listdir(f"data_RESULT/{case}_RESULT"):
			os.mkdir(f"data_RESULT/{case}_RESULT/GMB p={pGMBi}")
		...
		for prec, title in zip(data_prec, metrics):
			...
			plt.savefig(f"{folder}_RESULT/{case}_RESULT/GMB p={pGMBi}/{case}_GRAPH {title}.png")
			plt.close()
			...
			plt.savefig(f"{folder}_RESULT/{case}_RESULT/GMB p={pGMBi}/{case}_GRAPH_GLOBAL.png")
			plt.close()
	...
	plt.savefig(f"{folder}_RESULT/{case}_RESULT/{case}_GRAPH_DESC.png")
	plt.close()
	...
	if f"confusion" not in os.listdir(f"{folder}_RESULT/{case}_RESULT/"):
		os.mkdir(f"{folder}_RESULT/{case}_RESULT/confusion")
...
def all_view(folder):
	...
	for case in os.listdir(folder):
		...
		for langage in os.listdir(f"{folder}/{case}"):
			view(folder=folder, case=case, langage=langage)

		c.flm(f"Make {case} graph...")
		make_graph(folder=folder, case=case)
